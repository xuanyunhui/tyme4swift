...
public final class LunarYear: YearUnit, Tyme {
    ...
    private static let monthData: [String] = [
    ...
    private static let leapData: [[Int]] = {
        var result: [[Int]] = []
        for m in monthData {
            var n = 0
            let size = m.count / 2
            var list: [Int] = []
            for y in 0..<size {
                let z = m.index(m.startIndex, offsetBy: y * 2)
                var t = 0
                var c = 1
                for x in stride(from: 1, through: 0, by: -1) {
                    let idx = m.index(z, offsetBy: x)
                    let ch = m[idx]
                    if let v = chars.firstIndex(of: ch) {
                        t += c * chars.distance(from: chars.startIndex, to: v)
                    }
                    c *= 64
                }
                n += t
